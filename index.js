...
function submitToken() {
    let token = document.getElementById("message-text").value.trim();
    ...
    if (!tagCompanyIdPattern.test(tagCompanyId)) {
    ...
    localStorage.setItem("tagCompanyId", tagCompanyId)

    bootstrap.Modal
        .getInstance(document.getElementById("exampleModal"))
        .hide();

    fetchToken(token, clientId, orgId, companyId, tagCompanyId);
}

function fetchToken (token, clientId, orgId, companyId, tagCompanyId) {
    fetchProperties(token, clientId, orgId, companyId, tagCompanyId); // fetch properties using the token
}
...
function Submit () {
    ...
    cId = document.getElementById("tagCompanyId").value;
